...
./scripts/release.sh --no-mr         # skip merge request creation
...
The script (~700 lines) is organized into sequential modules, each responsible for one phase of the release workflow:

1. **Logging & utilities** — color-coded output (`log_info`, `log_warn`, `log_error`, `log_success`), `confirm()` prompt, `validate_semver()`
2. **Argument parsing** — `parse_args()` handles CLI flags (`--dry-run`, `--no-mr`, `--update-default-branch`, `--config`, `--help`)
...
8. **Git operations** — branch/tag creation with push
9. **Error recovery** — `cleanup_on_failure()` trap handler removes partial remote branches/tags on failure
10. **Main flow** — `main()` orchestrates the full workflow
